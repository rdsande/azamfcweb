<div class="uk-section">
    <div class="uk-container uk-container-medium games-fixtures">
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">AUGUST 2023</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2023/aug.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">SEPTEMBER 2023</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2023/sep.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">OCTOBER 2023</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2023/oct.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">NOVEMBER 2023</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2023/nov.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">DECEMBER 2023</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2023/dec.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">FEBRUARY 2024</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2024/feb.fixtures')
            </ul>
        </div>
        <!-- Monthly -->
        <div class="widget-header-wrapper">
            <div class="widget-header-wrapper__header">
                <header class="widget-header  ">
                    <h2 class="widget-header__title">MARCH 2024</h2>
                </header>
            </div>
        </div>
        <!-- Fixtures -->
        <div>
            <ul class="uk-grid-small uk-grid uk-child-width-1-3@s uk-text-center fxtures-list-divs">
                @include('includes/fixtures/2024/mar.fixtures')
            </ul>
        </div>
    </div>
</div>